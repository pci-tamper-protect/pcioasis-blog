...
{{< figure src="manifest-diff.png" alt="Side-by-side diff of v2.1.0 and v2.1.1 manifest.json showing new permissions highlighted" caption="The manifest diff between clean and compromised versions. Three additions — webRequest, tabs, and http://*/* — give the attacker universal network interception and broad host access." >}}
...
{{< figure src="c2-dashboard-multi-channel.png" alt="Lab 3 C2 dashboard showing simultaneous form, cookie, keystroke, and clipboard captures from a single session" caption="The Lab 3 C2 dashboard — a single user session produces captures across all five channels: form submission, keystrokes, session cookies, localStorage tokens, and clipboard events." >}}
...
{{< figure src="c2-server-payload.png" alt="Lab 3 C2 server logs showing incoming payloads classified by type: cookies, form_submission, localStorage" caption="C2 server output — each incoming payload is classified and logged. A single page visit to the checkout page produces cookies, form data, and localStorage tokens." >}}
...
{{< figure src="chrome-extensions-manager.png" alt="Chrome extension manager showing SecureForm Assistant v2.1.1 with expanded permissions panel" caption="Chrome Extensions manager showing SecureForm Assistant v2.1.1. The 'Details' panel shows 'Read and change all your data on all websites' — a red flag for a form validation tool." >}}
...
{{< figure src="extension-devtools-sources.png" alt="Extension background page DevTools showing content.js with MALICIOUS_CONFIG block visible in Sources panel" caption="Extension DevTools — Sources panel showing content.js. The MALICIOUS_CONFIG block is visible immediately below the legitimate configuration, containing the C2 collectUrl." >}}
